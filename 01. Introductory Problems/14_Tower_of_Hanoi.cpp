...
void calculateMoves(ll numberOfDisks, ll src, ll dst, ll aux) {
  if (numberOfDisks == 1)
    ++moves;
  else {
    calculateMoves (numberOfDisks - 1, src, aux, dst);      // Step 1
    ++moves;                                                 // Step 2
    calculateMoves (numberOfDisks - 1, aux, dst, src);      // Step 3
  }
}

void towerOfHanoi(ll numberOfDisks, ll src, ll dst, ll aux) {
  if (numberOfDisks == 1)
    cout << src << ' ' << dst << '\n';
  else {
    towerOfHanoi (numberOfDisks - 1, src, aux, dst);      // Step 1
    cout << src << ' ' << dst << '\n';                    // Step 2
    towerOfHanoi (numberOfDisks - 1, aux, dst, src);      // Step 3
  }
}
...
int main(){
  ...
  ll numberOfDisks;
  cin >> numberOfDisks;
  calculateMoves(numberOfDisks, 1, 3, 2);
  cout << moves << '\n';
